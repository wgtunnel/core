...
actual class SystemDnsResolver actual constructor(val networkMonitor: NetworkMonitor) : PeerResolver {
    private val log = Logger.withTag("SystemDnsResolver")
    override suspend fun resolve(host: String): DnsBootstrapResult =
        withContext(Dispatchers.IO) {
            val network = networkMonitor.networkState.value?.network ?: run {
            ...
            try {
                val ips =
                    withTimeoutOrNull(DNS_REQUEST_TIMEOUT_SEC.seconds) {
                        ...
                        log.w { "Bootstrap timed out after $DNS_REQUEST_TIMEOUT_SEC seconds, returning empty result" }
                        return@withContext DnsBootstrapResult()
                    }
                DnsBootstrapResult(
